...
#define HID_UNIT_EXPONENT8(a) HID_ITEM(HID_ITEM_TAG_UNIT_EXPONENT, HID_ITEM_TYPE_GLOBAL, 1), a
...
#error "A proper consumer HID report usage range must be selected"
    ...
    // QMK-style hires scroll: the host reads the physical metadata below and
    // multiplies every wheel unit by PHYSICAL_MAX. The firmware divides wheel
    // deltas by the same value, so the total travel is unchanged and the host
    // receives fine-grained events it can interpolate.
    ...
    HID_PHYSICAL_MAX8(CONFIG_ZMK_POINTING_SMOOTH_SCROLL_DIVISOR),
    HID_UNIT_EXPONENT8(0x00),

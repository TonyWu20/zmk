...
void zmk_pointing_resolution_multipliers_set_profile(struct zmk_pointing_resolution_multipliers m,
    ...
    // operation involving this state must be atomic. Currently, each function either reads or
    // writes only one entry at a time, so it is safe to do these operations without a lock.
...
void zmk_pointing_resolution_multipliers_process_report(
    ...
    LOG_DBG("Update resolution multipliers: endpoint=%d, resolution=0x%02x",
            endpoint.transport, vals.resolution);
}
